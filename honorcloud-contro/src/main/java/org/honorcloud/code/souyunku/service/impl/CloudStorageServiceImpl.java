...
import org.honorcloud.code.dao.base.BaseUtils;
import org.honorcloud.code.dao.base.YmqBaseDao;
import org.honorcloud.code.souyunku.model.base.QueryResult;
import org.honorcloud.code.souyunku.model.base.ResultModel;
import org.honorcloud.code.souyunku.model.dto.ImagesDTO;
...
import org.honorcloud.code.souyunku.service.storage.CloudStorageFactory;
...
public class CloudStorageServiceImpl implements CloudStorageService {
	...
	public ResultModel upload(MultipartFile file) throws Exception {
		ResultModel resultModel = new ResultModel(Constant.MessageEnum.UPLOAD_FAILURE.getKey(),
				Constant.MessageEnum.SUCCESS.getValue(), null);
		try {
			String url = "";
			...
			resultModel = new ResultModel(
					StringUtils.isNotBlank(url) == true ? Constant.MessageEnum.SUCCESS.getKey()
							: Constant.MessageEnum.UPLOAD_FAILURE.getKey(),
					StringUtils.isNotBlank(url) == true ? Constant.MessageEnum.SUCCESS.getValue()
							: Constant.MessageEnum.UPLOAD_FAILURE.getValue(),
					resultMap);
	...
	public HashMap<String, Object> selectListAndCount(String params, String pageNum, String pageSize) {
		...
		try {
			...
			QueryResult<ImagesDTO> queryResult = ymqBaseDao.selectListAndCount(
					BaseUtils.makeClazzPath(ImagesPo.class, "selectImagesList"), imagesPo, pNum, pSize,
					"supdate_time desc");
	...
	public ResultModel<Integer> update(ImagesPo imagesPo) {
		...
		ResultModel resultModel = new ResultModel(Constant.MessageEnum.UPDATE_FAILURE.getKey(),
				Constant.MessageEnum.UPDATE_FAILURE.getValue(), null);
		...
		try {
			...
			resultModel = new ResultModel(
					count > 0 ? Constant.MessageEnum.SUCCESS.getKey() : Constant.MessageEnum.UPDATE_FAILURE.getKey(),
					count > 0 ? Constant.MessageEnum.SUCCESS.getValue()
							: Constant.MessageEnum.UPDATE_FAILURE.getValue(),
					count);
	...
	public ResultModel<Integer> deleteById(Integer imagesId) {
		...
		ResultModel resultModel = new ResultModel(Constant.MessageEnum.DELETE_FAILURE.getKey(),
				Constant.MessageEnum.DELETE_FAILURE.getValue(), null);
		...
		try {
			...
			/*
			 * ResultModel<CloudStorageConfigPo> cloudStorageConfig =
			 * sysConfigService.selectCloudStorageConfig();
			 * 
			 * if
			 * (!cloudStorageConfig.getRetCode().equals(Constant.MessageEnum.SUCCESS.getKey(
			 * ))) { return new ResultModel(cloudStorageConfig.getRetCode(),
			 * cloudStorageConfig.getRetMsg(), null); }
			 * 
			 * ImagesPo images = ymqBaseDao.selectOne(imagesPo);
			 * 
			 * CloudStorageFactory.build(cloudStorageConfig.getResult()).deleteObject(images
			 * .getUrl());
			 */
			...
			resultModel = new ResultModel(
					count > 0 ? Constant.MessageEnum.SUCCESS.getKey() : Constant.MessageEnum.DELETE_FAILURE.getKey(),
					count > 0 ? Constant.MessageEnum.SUCCESS.getValue()
							: Constant.MessageEnum.DELETE_FAILURE.getValue(),
					count);
	...
	public ResultModel<Integer> insertImages(ImagesPo imagesPo) {
		ResultModel resultModel = new ResultModel(Constant.MessageEnum.ADD_FAILURE.getKey(),
				Constant.MessageEnum.ADD_FAILURE.getValue(), null);
		try {
			...
			return new ResultModel(
					count > 0 ? Constant.MessageEnum.SUCCESS.getKey() : Constant.MessageEnum.ADD_FAILURE.getKey(),
					count > 0 ? Constant.MessageEnum.SUCCESS.getValue() : Constant.MessageEnum.ADD_FAILURE.getValue(),
					count);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			if (config != null) {
				...
				return new ResultModel(
						count > 0 ? Constant.MessageEnum.SUCCESS.getKey() : Constant.MessageEnum.ADD_FAILURE.getKey(),
						count > 0 ? Constant.MessageEnum.SUCCESS.getValue()
								: Constant.MessageEnum.ADD_FAILURE.getValue(),
						count);
			}
	...
	public ResultModel<Integer> defaultCloudStorageType(String storageType) throws Exception {
		...
		try {
			...
			if (config == null) {
				return new ResultModel(Constant.MessageEnum.CLOUD_STORAGE_IS_EMPTY.getKey(),
						Constant.MessageEnum.CLOUD_STORAGE_IS_EMPTY.getValue(), null);
			}
